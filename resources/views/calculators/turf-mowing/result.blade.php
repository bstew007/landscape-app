...
<div class="max-w-5xl mx-auto py-8 px-4">
    ...
    <div class="mb-8">
        <div class="flex items-center gap-3 mb-3">
            ...
            <h1 class="text-4xl font-bold text-gray-900">Turf Maintenance Calculation Results</h1>
        </div>
        <p class="text-gray-600">Review your calculation summary and import to an estimate.</p>
    </div>
        ...
        <div class="p-6">
            <div class="space-y-3 mb-6">
                @foreach ($data['labor_by_task'] as $task => $hours)
                    <div class="flex justify-between items-center py-3 border-b border-gray-100 last:border-0">
                        <span class="text-gray-700 font-medium capitalize">{{ str_replace('_', ' ', $task) }}</span>
            ...
            <div class="bg-gradient-to-r from-gray-50 to-gray-100 rounded-lg p-4 space-y-2">
                ...
                <div class="flex justify-between text-sm">
                    ...
                    <span class="text-gray-900 font-semibold">{{ number_format($data['overhead_hours'], 2) }} hrs</span>
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-gray-600">Drive Time:</span>
                    <span class="text-gray-900 font-semibold">{{ number_format($data['drive_time_hours'], 2) }} hrs</span>
                </div>
        ...
        <div class="p-6">
            <div class="space-y-3">
                ...
                <div class="flex justify-between py-2 border-t border-gray-200">
                    ...
                    <span class="text-gray-900 font-semibold">${{ number_format(($data['labor_cost'] ?? 0) + ($data['material_total'] ?? 0), 2) }}</span>
                </div>
    ...
    @if(isset($calculation) && $siteVisit)
        <div class="bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden mb-6" 
             x-data="{ 
                ...
                newEstimateTitle: 'Turf Maintenance - {{ date('M d, Y') }}',
                areaName: 'Turf Maintenance - {{ date('M d, Y') }}'
             }">
            <div class="bg-gradient-to-r from-gray-800 to-gray-700 px-6 py-4 border-b border-gray-600">
                ...
                <div class="mb-6">
                    ...
                    <div x-show="estimateMode === 'new'" x-cloak>
                        <input type="text" 
                               ...
                               class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-accent-500 focus:border-transparent transition"
                               placeholder="e.g., Turf Maintenance - Weekly Service">
                    ...
                    <div class="space-y-3">
                        <label class="flex items-start p-4 border-2 border-gray-200 rounded-lg hover:border-brand-400 hover:bg-brand-50 cursor-pointer transition group">
                            ...
                            <div class="ml-4 flex-1">
                                ...
                                <p class="text-sm text-gray-600 mt-1">Creates separate line items for each task with full detail (mowing, trimming, edging, drive time, overhead)</p>
                            </div>
                        </label>
                        <label class="flex items-start p-4 border-2 border-gray-200 rounded-lg hover:border-gray-300 hover:bg-gray-50 cursor-pointer transition">
                            ...
                            <div class="ml-4 flex-1">
                                ...
                                <p class="text-sm text-gray-600 mt-1">Creates single labor line item (old format)</p>
                            </div>
                        </label>
                    </div>
                </div>
    ...
    <div class="flex flex-col sm:flex-row gap-4">
        @if(isset($calculation))
            @php $downloadUrl = route('calculators.turf_mowing.downloadPdf', $calculation->id); @endphp
            <a href="{{ $downloadUrl }}" 
