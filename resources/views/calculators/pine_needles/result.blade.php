...
<div class="max-w-5xl mx-auto py-8 px-4">
    ...
    <div class="mb-8">
        <div class="flex items-center gap-3 mb-3">
            ...
            <h1 class="text-4xl font-bold text-gray-900">Pine Needles Calculation Results</h1>
        </div>
        ...
        <div class="p-6">
            <div class="space-y-3">
                ...
                <div class="flex justify-between py-2">
                    ...
                    <span class="text-gray-900 font-semibold">${{ number_format($data['material_total'] ?? 0, 2) }}</span>
                </div>
                <div class="flex justify-between py-2 border-t border-gray-200">
                    <span class="text-gray-900 font-semibold">Total Cost:</span>
                    <span class="text-gray-900 font-semibold">${{ number_format($data['labor_cost'] + ($data['material_total'] ?? 0), 2) }}</span>
                </div>
    ...
    @if(isset($calculation) && $siteVisit)
        <div class="bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden mb-6" 
             x-data="{ 
                ...
                newEstimateTitle: 'Pine Needles - {{ date('M d, Y') }}',
                areaName: 'Pine Needles - {{ date('M d, Y') }}'
             }">
            <div class="bg-gradient-to-r from-gray-800 to-gray-700 px-6 py-4 border-b border-gray-600">
                ...
                <div class="mb-6">
                    ...
                    <div x-show="estimateMode === 'new'" x-cloak>
                        <input type="text" 
                               ...
                               class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-accent-500 focus:border-transparent transition"
                               placeholder="e.g., Pine Needles - Front Beds">
                    ...
                    <div class="space-y-3">
                        <label class="flex items-start p-4 border-2 border-gray-200 rounded-lg hover:border-brand-400 hover:bg-brand-50 cursor-pointer transition group">
                            ...
                            <div class="ml-4 flex-1">
                                ...
                                <p class="text-sm text-gray-600 mt-1">Creates separate line items for each task with full detail (installation, drive time, overhead)</p>
                            </div>
                        </label>
    ...
    <div class="flex flex-col sm:flex-row gap-4">
        @if(isset($calculation))
            @php $downloadUrl = route('calculators.pine_needles.downloadPdf', $calculation->id); @endphp
            <a href="{{ $downloadUrl }}" 
