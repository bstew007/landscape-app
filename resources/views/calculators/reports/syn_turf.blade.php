...
<section class="bg-white p-6 rounded shadow space-y-4">
    <div class="flex items-center justify-between">
        <h2 class="text-2xl font-semibold">🏟️ Synthetic Turf</h2>
        <span class="text-lg font-bold text-green-700">${{ number_format($data['final_price'], 2) }}</span>
    </div>

    <div class="grid md:grid-cols-2 gap-4">
        <div class="bg-gray-50 p-4 rounded">
            <h4 class="font-semibold mb-2">Project Inputs</h4>
            <p>Area: {{ number_format($data['area_sqft'] ?? 0, 2) }} sq ft</p>
            <p>Edging: {{ number_format($data['edging_linear_ft'] ?? 0, 2) }} ft</p>
            <p>Turf: {{ $data['turf_name'] ?? '—' }} @ ${{ number_format($data['turf_unit_cost'] ?? 0, 2) }}/sq ft</p>
        </div>
        <div class="bg-gray-50 p-4 rounded">
