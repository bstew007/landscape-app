...
@section('content')
@php
    $categories = \App\Models\MaterialCategory::where('is_active', true)->orderBy('name', 'asc')->get();
    $selectedCategories = $material->categories->pluck('id')->toArray();
...
return {
    // Inputs
    unitCost: {{ old('unit_cost', $material->unit_cost ?? 0) }},
    taxRate: {{ old('tax_rate', $material->tax_rate ?? 0) }},
    overheadMarkup: {{ number_format($overheadMarkup, 2, '.', '') }},
    // Pricing
    mode: '{{ old('pricing_mode', $material->pricing_mode ?? 'budget') }}',
    budgetMargin: {{ number_format($budgetMargin * 100, 1, '.', '') }},
    customMargin: {{ old('custom_margin', $material->custom_margin ?? number_format($budgetMargin * 100, 1, '.', '')) }},
    customPrice: {{ old('unit_price', $material->unit_price ?? 0) }},
...
<div class="space-y-8 max-w-7xl mx-auto p-4">
        ...
        <section class="rounded-[32px] bg-gradient-to-br from-brand-900 via-brand-800 to-brand-700 text-white p-6 sm:p-8 shadow-2xl border border-brand-800/40 relative overflow-hidden">
            <div class="flex flex-wrap items-start gap-6">
                <div class="space-y-3 max-w-3xl">
                    ...
                    <h1 class="text-3xl sm:text-4xl font-semibold">Edit Material</h1>
                    <p class="text-sm text-brand-100/85">Update material with cost, tax, overhead, and profit margins.</p>
                </div>
                <div class="ml-auto flex gap-2">
                    ...
                    <button form="materialEditForm" type="submit" class="inline-flex items-center h-9 px-4 rounded-lg bg-white text-brand-900 text-sm font-semibold hover:bg-brand-50">Update Material</button>
                </div>
            </div>
    ...
    <section class="rounded-[32px] bg-white shadow-2xl border border-brand-100/60 overflow-hidden p-6 sm:p-8">
        ...
        <form id="materialEditForm" method="POST" action="{{ route('materials.update', $material) }}" class="space-y-6">
            @csrf
            @method('PUT')
                    ...
                    <x-panel-card title="Item Information" titleClass="text-lg font-semibold text-gray-900 mb-4" class="space-y-4">
                        <div class="grid grid-cols-1 sm:grid-cols-3 items-center gap-4">
                            ...
                            <div class="sm:col-span-2">
                                <input id="name" type="text" name="name" class="form-input w-full" value="{{ old('name', $material->name) }}" required>
                            </div>
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-3 items-center gap-4">
                            <label for="sku" class="text-sm font-medium text-gray-800 sm:text-right whitespace-nowrap">SKU</label>
                            <div class="sm:col-span-2">
                                <input id="sku" type="text" name="sku" class="form-input w-full" value="{{ old('sku', $material->sku) }}">
                            </div>
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-3 items-center gap-4">
                            <label for="unit" class="text-sm font-medium text-gray-800 sm:text-right whitespace-nowrap">Unit</label>
                            <div class="sm:col-span-2">
                                <input id="unit" type="text" name="unit" class="form-input w-full" value="{{ old('unit', $material->unit) }}" required>
                            </div>
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-3 items-center gap-4">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 items-center gap-4">
                            ...
                            <div class="sm:col-span-2">
                                <input id="vendor_sku" type="text" name="vendor_sku" class="form-input w-full" value="{{ old('vendor_sku', $material->vendor_sku) }}">
                            </div>
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-3 items-start gap-4">
                            ...
                            <div class="sm:col-span-2">
                                <textarea id="description" name="description" rows="2" class="form-textarea w-full" placeholder="Displayed when selecting materials">{{ old('description', $material->description) }}</textarea>
                            </div>
                        </div>
                    </x-panel-card>
                </div>
                ...
                <div class="space-y-6">
                    <x-panel-card title="Cost + Breakeven" titleClass="text-lg font-semibold text-gray-900 mb-4" class="space-y-4">
                        <div class="grid grid-cols-1 sm:grid-cols-3 items-center gap-4">
                            ...
                            <div class="sm:col-span-2">
                                <input id="unit_cost" type="number" step="0.01" min="0" name="unit_cost" class="form-input w-full" x-model.number="unitCost" value="{{ old('unit_cost', $material->unit_cost) }}" required>
                            </div>
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-3 items-center gap-4">
                            <label for="tax_rate" class="text-sm font-medium text-gray-800 sm:text-right whitespace-nowrap">Purchase Tax %</label>
                            <div class="sm:col-span-2">
                                <input id="tax_rate" type="number" step="0.001" min="0" name="tax_rate" class="form-input w-full" x-model.number="taxRate" value="{{ old('tax_rate', $material->tax_rate) }}">
                            </div>
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-3 items-center gap-4">
            ...
            <div class="flex items-center gap-6 pt-2">
                <label class="inline-flex items-center">
                    <input type="checkbox" name="is_taxable" value="1" class="form-checkbox" {{ old('is_taxable', $material->is_taxable) ? 'checked' : '' }}>
                    <span class="ml-2 text-sm">Taxable (to customer)</span>
                </label>
                <label class="inline-flex items-center">
                    <input type="checkbox" name="is_active" value="1" class="form-checkbox" {{ old('is_active', $material->is_active) ? 'checked' : '' }}>
            ...
            <div class="border-t pt-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">Categories ({{ $material->categories->count() }})</h3>
