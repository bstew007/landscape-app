...
<script>
function laborEditForm(){
return {
    // Inputs
    wage: {{ old('average_wage', $labor->average_wage ?? 0) }},
    otFactor: {{ old('overtime_factor', $labor->overtime_factor ?? 1.00) }},
    unbillable: {{ old('unbillable_percentage', $labor->unbillable_percentage ?? 0) }},
    burden: {{ old('labor_burden_percentage', $labor->labor_burden_percentage ?? 0) }},
    overhead: {{ number_format($ohr, 2, '.', '') }},
    // Pricing
    mode: '{{ old('pricing_mode', $labor->pricing_mode ?? 'budget') }}',
    budgetMargin: {{ number_format($budgetMargin * 100, 1, '.', '') }},
    customMargin: {{ old('custom_margin', $labor->custom_margin ?? number_format($budgetMargin * 100, 1, '.', '')) }},
    customPrice: {{ old('base_rate', $labor->base_rate ?? 0) }},
...
<div class="space-y-8 max-w-7xl mx-auto p-4">
    <div x-data="laborEditForm()">
        ...
        <section class="rounded-[32px] bg-gradient-to-br from-brand-900 via-brand-800 to-brand-700 text-white p-6 sm:p-8 shadow-2xl border border-brand-800/40 relative overflow-hidden">
            <div class="flex flex-wrap items-start gap-6">
                <div class="space-y-3 max-w-3xl">
                    ...
                    <h1 class="text-3xl sm:text-4xl font-semibold">Edit Labor Item</h1>
                    <p class="text-sm text-brand-100/85">Update labor catalog item with wage calculations, overhead recovery, and profit margins.</p>
                </div>
                <div class="ml-auto flex gap-2">
                    ...
                    <button form="laborEditForm" type="submit" class="inline-flex items-center h-9 px-4 rounded-lg bg-white text-brand-900 text-sm font-semibold hover:bg-brand-50">Update Labor Item</button>
                </div>
            </div>
        ...
        <section class="rounded-[32px] bg-white shadow-2xl border border-brand-100/60 p-6 sm:p-8 space-y-6">
            ...
            <form id="laborEditForm" method="POST" action="{{ route('labor.update', $labor) }}" class="space-y-6">
                @csrf
                @method('PUT')
                <input type="hidden" name="type" value="{{ old('type', $labor->type ?? 'crew') }}">
                ...
                <input type="hidden" name="pricing_mode" x-model="mode">
                        ...
                        <x-panel-card title="Item Information" titleClass="text-lg font-semibold text-gray-900 mb-4" class="space-y-4">
                            <div class="grid grid-cols-1 sm:grid-cols-3 items-center gap-4">
                                ...
                                <div class="sm:col-span-2">
                                    <input id="name" type="text" name="name" class="form-input w-full" value="{{ old('name', $labor->name) }}" required>
                                </div>
                            </div>
                            <div class="grid grid-cols-1 sm:grid-cols-3 items-center gap-4">
                                <label for="unit" class="text-sm font-medium text-gray-800 sm:text-right whitespace-nowrap">Units</label>
                                <div class="sm:col-span-2">
                                    <input id="unit" type="text" name="unit" class="form-input w-full" value="{{ old('unit', $labor->unit ?? 'hr') }}" required>
                                </div>
                            </div>
                            <div class="grid grid-cols-1 sm:grid-cols-3 items-start gap-4">
                                ...
                                <div class="sm:col-span-2">
                                    <textarea id="description" name="description" rows="2" class="form-textarea w-full" placeholder="Client-facing description">{{ old('description', $labor->description) }}</textarea>
                                </div>
                            </div>
                            <div class="grid grid-cols-1 sm:grid-cols-3 items-start gap-4">
                                ...
                                <div class="sm:col-span-2">
                                    <textarea id="internal_notes" name="internal_notes" rows="2" class="form-textarea w-full" placeholder="Internal only">{{ old('internal_notes', $labor->internal_notes) }}</textarea>
                                </div>
                            </div>
                        </x-panel-card>
                    </div>
                    ...
                    <div class="space-y-6">
                        <x-panel-card title="Cost + Breakeven" titleClass="text-lg font-semibold text-gray-900 mb-4" class="space-y-4">
                            <div class="grid grid-cols-1 sm:grid-cols-3 items-center gap-4">
                                ...
                                <div class="sm:col-span-2">
                                    <div class="relative">
                                        <input id="average_wage" type="number" step="0.01" min="0" name="average_wage" class="form-input w-full pr-11" x-model.number="wage" value="{{ old('average_wage', $labor->average_wage) }}">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-3 items-center gap-4">
                                ...
                                <div class="sm:col-span-2">
                                    <input id="overtime_factor" type="number" step="0.01" min="1" name="overtime_factor" class="form-input w-full" x-model.number="otFactor" value="{{ old('overtime_factor', $labor->overtime_factor ?? 1.00) }}" placeholder="e.g., 1.5">
                                </div>
                            </div>
                            <div class="grid grid-cols-1 sm:grid-cols-3 items-center gap-4">
                                <label for="unbillable_percentage" class="text-sm font-medium text-gray-800 sm:text-right whitespace-nowrap">Unbillable %</label>
                                <div class="sm:col-span-2">
                                    <input id="unbillable_percentage" type="number" step="0.1" min="0" max="99.9" name="unbillable_percentage" class="form-input w-full" x-model.number="unbillable" value="{{ old('unbillable_percentage', $labor->unbillable_percentage ?? 0) }}">
                                </div>
                            </div>
                            <div class="grid grid-cols-1 sm:grid-cols-3 items-center gap-4">
                                ...
                                <div class="sm:col-span-2">
                                    <input id="labor_burden_percentage" type="number" step="0.1" min="0" name="labor_burden_percentage" class="form-input w-full" x-model.number="burden" value="{{ old('labor_burden_percentage', $labor->labor_burden_percentage ?? 0) }}">
                                </div>
                            </div>
                            <div class="grid grid-cols-1 sm:grid-cols-3 items-center gap-4">
                ...
                <div class="flex items-center gap-6 pt-2">
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="is_billable" value="1" class="form-checkbox" {{ old('is_billable', $labor->is_billable ?? true) ? 'checked' : '' }}>
                        <span class="ml-2 text-sm">Billable</span>
                    </label>
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="is_active" value="1" class="form-checkbox" {{ old('is_active', $labor->is_active ?? true) ? 'checked' : '' }}>
                        <span class="ml-2 text-sm">Active</span>
                    </label>
                </div>

                <!-- Hidden/Advanced (optional) -->
                <input type="hidden" name="overtime_rate" value="{{ old('overtime_rate', $labor->overtime_rate) }}">
                <input type="hidden" name="cost_code_id" value="{{ old('cost_code_id', $labor->cost_code_id) }}">
            </form>
        </section>
    </div>
</div>
