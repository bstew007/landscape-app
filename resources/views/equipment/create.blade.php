...
@section('content')
<div x-data="{
    ownershipType: '{{ old('ownership_type', 'company') }}',
    unit: '{{ old('unit', 'hr') }}',
    hourlyCost: {{ old('hourly_cost', 0) ?: 0 }},
    dailyCost: {{ old('daily_cost', 0) ?: 0 }},
    hourlyRate: {{ old('hourly_rate', 0) ?: 0 }},
    dailyRate: {{ old('daily_rate', 0) ?: 0 }},
    profitPercent: {{ old('profit_percent', 20) }},
    ...
    <section class="rounded-[20px] sm:rounded-[28px] lg:rounded-[32px] bg-gradient-to-br from-brand-900 via-brand-800 to-brand-700 text-white p-4 sm:p-6 lg:p-8 shadow-2xl border border-brand-800/40">
        <div class="flex flex-wrap items-start gap-4 sm:gap-6">
            <div class="space-y-2 sm:space-y-3 max-w-3xl">
                ...
                <h1 class="text-2xl sm:text-3xl lg:text-4xl font-semibold">Add Equipment</h1>
                <p class="text-xs sm:text-sm text-brand-100/85">Add new equipment to the catalog for use in estimates.</p>
            </div>
            <div class="ml-auto flex gap-2">
                ...
                <button form="equipmentCreateForm" type="submit" class="inline-flex items-center h-9 px-4 rounded-lg bg-white text-brand-900 text-sm font-semibold hover:bg-brand-50">Save Equipment</button>
            </div>
        </div>
    </section>
    ...
    <section class="rounded-[32px] bg-white shadow-2xl border border-brand-100/60 overflow-hidden">
        <form id="equipmentCreateForm" method="POST" action="{{ route('equipment.store') }}" class="p-6 sm:p-8 space-y-8">
            @csrf
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3"
                              class="w-full rounded-lg border-brand-200 focus:border-brand-500 focus:ring focus:ring-brand-500/20"
                              placeholder="Equipment description for estimates">{{ old('description') }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="2"
                              class="w-full rounded-lg border-brand-200 focus:border-brand-500 focus:ring focus:ring-brand-500/20"
                              placeholder="Internal notes (not shown on estimates)">{{ old('notes') }}</textarea>
                </div>
