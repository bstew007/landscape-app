...
<div class="max-w-3xl mx-auto py-8 px-4">
    <div class="mb-6">
        <a href="{{ route('estimates.show', $estimate) }}?tab=print" class="inline-flex items-center text-sm text-gray-600 hover:text-gray-900">
    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        <div class="bg-brand-600 px-6 py-4">
            <h1 class="text-xl font-semibold text-white">Email Estimate - {{ $templateLabel }}</h1>
            <p class="text-sm text-brand-100 mt-1">Review and send estimate to client</p>
        </div>

        <form method="POST" action="{{ route('estimates.send-email', $estimate) }}" class="p-6 space-y-6">
            @csrf
            ...
            <div>
                ...
                <input type="email" 
                       ...
                       value="{{ old('recipient_email', $estimate->client->email ?? '') }}"
                       required
            ...
            <div>
                ...
                <p class="mt-1 text-xs text-gray-500">Personalize your message to the client</p>
                @error('message')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                <h3 class="text-sm font-semibold text-gray-900 mb-3">Estimate Details</h3>
                <dl class="grid grid-cols-2 gap-3 text-sm">
                    <div>
                        <dt class="text-gray-600">Estimate #</dt>
                        <dd class="font-medium text-gray-900">{{ $estimate->id }}</dd>
                    </div>
                    <div>
                        <dt class="text-gray-600">Template</dt>
                        <dd class="font-medium text-gray-900">{{ $templateLabel }}</dd>
                    </div>
                    <div>
                        <dt class="text-gray-600">Client</dt>
                        <dd class="font-medium text-gray-900">{{ $estimate->client->name }}</dd>
                    </div>
                    <div>
                        <dt class="text-gray-600">Total</dt>
                        <dd class="font-medium text-gray-900">${{ number_format($estimate->grand_total ?? 0, 2) }}</dd>
                    </div>
                </dl>
            </div>
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-blue-700 mt-1">
                            A PDF of the {{ $templateLabel }} estimate will be automatically attached to this email.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center justify-between gap-4 pt-4 border-t">
                <a href="{{ route('estimates.print', $estimate) }}?template={{ $template }}" 
                   target="_blank"
                ...
                <div class="flex items-center gap-3">
                    <a href="{{ route('estimates.show', $estimate) }}?tab=print" 
