                    ...
                    <select name="category" id="category" required 
                        ...
                        @foreach($mappings as $mapping)
                            <option value="{{ $mapping->category }}" 
                                ...
                                {{ old('category', $expense->category) == $mapping->category ? 'selected' : '' }}>
                        ...
                        @if(!empty($qboAccounts))
                            @foreach($qboAccounts as $account)
                                <option value="{{ $account['Id'] }}" 
                                    ...
                                    {{ old('qbo_account_id', $expense->qbo_account_id) == $account['Id'] ? 'selected' : '' }}>
                                    {{ $account['Name'] }}
                                </option>
                            @endforeach
                        @endif
                        ...
                        <select name="vendor" 
                            ...
                            @foreach($vendors as $vendor)
                                ...
                                <option value="{{ $vendorName }}" 
                                    {{ old('vendor', $expense->vendor) == $vendorName ? 'selected' : '' }}>
                                    {{ $vendorName }}
                                </option>
                            @endforeach
                        </select>
    ...
    <script>
        ...
        function toggleIssueField() {
            ...
            if (qboAccountSelect && mappedAccountHint && accountMappings[category]) {
                // Only auto-select if no account is already selected
                if (!qboAccountSelect.value || qboAccountSelect.value === '') {
