...
<div class="space-y-8">
    <section class="rounded-[32px] bg-gradient-to-br from-brand-900 via-brand-800 to-brand-700 text-white p-6 sm:p-8 shadow-2xl border border-brand-800/40 relative overflow-hidden">
        <div class="flex flex-wrap items-start gap-6">
            <div class="space-y-2 max-w-2xl">
                ...
                <h1 class="text-2xl sm:text-3xl font-semibold">Edit Contact Tag</h1>
                <p class="text-sm text-brand-100/90">Update tag details and styling.</p>
            </div>
        </div>
    </section>
    ...
    <section class="rounded-[32px] bg-white shadow-2xl border border-brand-100/60 overflow-hidden">
        <div class="p-6 sm:p-8">
            <form method="POST" action="{{ route('admin.contact-tags.update', $tag) }}" class="space-y-6">
                @csrf
                @method('PUT')
                    ...
                    <div class="rounded-lg bg-brand-50 border border-brand-200 px-4 py-3">
                        <div class="flex items-start gap-2">
                            ...
                            <div class="text-sm text-brand-800">
                                <p class="font-semibold">This tag is assigned to {{ $tag->contacts()->count() }} contacts.</p>
                                <p class="text-xs mt-1">Changes to the name or color will be reflected on all tagged contacts.</p>
                            </div>
                        </div>
                    </div>
                @endif
