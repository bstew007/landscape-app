export function materialsEditor(root) {
  ...
  const rowsRef = () => (Array.isArray(root?.materialsRows) ? root.materialsRows : (Array.isArray(window.__initialMaterialsRows) ? window.__initialMaterialsRows : []));
  const getTaxPct = () => (typeof root?.materialsTaxPct !== 'undefined' ? root.materialsTaxPct : Number(window.__initialMaterialsTaxPct || 0));
  const setTaxPct = (v) => { if (typeof root?.materialsTaxPct !== 'undefined') root.materialsTaxPct = Number(v || 0); };
  const getIndustryAvg = () => (typeof root?.materialsIndustryAvgRatio !== 'undefined' ? root.materialsIndustryAvgRatio : Number(window.__initialMaterialsIndustryAvg || 22.3));
  const setIndustryAvg = (v) => { if (typeof root?.materialsIndustryAvgRatio !== 'undefined') root.materialsIndustryAvgRatio = Number(v || 0); };
  ...
  return {
    ...
    materialsPrevRatio(){ const sales = typeof root?.forecastTotal === 'function' ? root.forecastTotal() : 0; if (!sales) return 0; return (this.materialsPrevTotal() / Math.abs(sales)) * 100; },
    materialsRatio(){ const sales = typeof root?.forecastTotal === 'function' ? root.forecastTotal() : 0; if (!sales) return 0; return (this.materialsCurrentTotal() / Math.abs(sales)) * 100; },
  };
}
