...
class QboVendorService
{
    ...
    public function upsert(Contact $c): array
    {
        ...
        $base = [
            'DisplayName' => $c->company_name ?: trim($c->first_name.' '.$c->last_name) ?: ($c->email ?: 'Vendor '.($c->id)),
            ...
            // Note: Mobile field for Vendor API
        ...
        $url = $this->baseUrl($token->realm_id).'/vendor';

        $isUpdate = (bool) $c->qbo_vendor_id;
        ...
        if ($isUpdate) {
            ...
            $get = Http::withHeaders($this->authHeaders())
                ->get($this->baseUrl($token->realm_id).'/vendor/'.$c->qbo_vendor_id, ['minorversion' => 65]);
            if (config('qbo.debug')) {
                \Log::info('QBO Vendor fetch before update', [
                    'status' => $get->status(),
            ...
            if ($get->status() === 401) {
                ...
                $get = Http::withHeaders($this->authHeaders())
                    ->get($this->baseUrl($token->realm_id).'/vendor/'.$c->qbo_vendor_id, ['minorversion' => 65]);
                if (config('qbo.debug')) {
                    \Log::info('QBO Vendor fetch retry after refresh', [
                        'status' => $get->status(),
                        'tid' => $get->header('intuit_tid'),
                        'body' => $get->body(),
                    ]);
            ...
            if ($get->ok()) {
                $existing = $get->json()['Vendor'] ?? null;
            } else if ($get->status() === 401) {
                throw new \RuntimeException('QBO auth failed while fetching vendor for update');
            }
            ...
            if (empty($updateBody)) {
                ...
                if ($excludedChanged) {
                    return ['Vendor' => $existing ?: ['Id' => $c->qbo_vendor_id], 'skipped' => 'excluded_fields_changed'];
                }
                ...
                return ['Vendor' => $existing ?: ['Id' => $c->qbo_vendor_id]];
            }
        ...
        if (config('qbo.debug')) {
            try {
                ...
                \Log::info('QBO Vendor upsert request (payload keys)', [
                    'is_update' => $isUpdate,
        ...
        if (config('qbo.debug')) {
            \Log::info('QBO Vendor upsert response', [
                'status' => $res->status(),
        ...
        if ($res->status() === 401 || str_contains($res->body(), 'Token expired')) {
            ...
            $res = Http::withHeaders($this->authHeaders())
                ...
                ->post($url, $payload);
            if (config('qbo.debug')) {
                \Log::warning('QBO Vendor upsert retry after refresh', [
                    'status' => $res->status(),
                    'tid' => $res->header('intuit_tid'),
                    'body' => $res->body(),
                ]);
        ...
        if (!$res->ok()) {
            throw new \RuntimeException('QBO Vendor upsert failed: '.$res->body());
        }

        $vendor = $res->json()['Vendor'] ?? null;
        if ($vendor) {
            $c->qbo_vendor_id = $vendor['Id'] ?? $c->qbo_vendor_id;
            $c->qbo_sync_token = $vendor['SyncToken'] ?? $c->qbo_sync_token;
            $c->qbo_last_synced_at = now();
    ...
    {
        if (!$c->qbo_vendor_id) throw new \RuntimeException('Contact is not linked to QBO as vendor');
        $token = QboToken::latest('updated_at')->first();
        if (!$token) throw new \RuntimeException('QBO not connected');
        ...
        $get = Http::withHeaders($this->authHeaders())
            ->get($this->baseUrl($realmId).'/vendor/'.$c->qbo_vendor_id, ['minorversion' => 65]);
        if ($get->status() === 401) {
            $this->refreshTokenIfNeeded();
            $get = Http::withHeaders($this->authHeaders())
                ->get($this->baseUrl($realmId).'/vendor/'.$c->qbo_vendor_id, ['minorversion' => 65]);
        ...
        $url = $this->baseUrl($realmId).'/vendor';
        $res = Http::withHeaders($this->authHeaders())
            ->withOptions(['query' => $query])
            ->post($url, $payload);
        if ($res->status() === 401) {
        ...
        if (!$res->ok()) throw new \RuntimeException('QBO Vendor Mobile update failed: '.$res->body());
        $vendor = $res->json()['Vendor'] ?? null;
        if ($vendor) {
            $c->qbo_sync_token = $vendor['SyncToken'] ?? $c->qbo_sync_token;
            $c->qbo_last_synced_at = now();
    ...
    public function updateNames(Contact $c): array
    {
        ...
        if (!$res->ok()) {
            ...
            throw new \RuntimeException('QBO Vendor name update failed: '.$res->body());
        }
        $vendor = $res->json()['Vendor'] ?? null;
        if ($vendor) {
            $c->qbo_sync_token = $vendor['SyncToken'] ?? $c->qbo_sync_token;
    ...
    public function fetch(Contact $c): array
    {
        if (!$c->qbo_vendor_id) throw new \RuntimeException('Contact is not linked to QBO as vendor');
        $token = QboToken::latest('updated_at')->first();
        if (!$token) throw new \RuntimeException('QBO not connected');
        ...
        $url = $this->baseUrl($realmId).'/vendor/'.$c->qbo_vendor_id;
        ...
        if (!$res->ok()) {
            throw new \RuntimeException('QBO vendor fetch failed: '.$res->body());
        }
