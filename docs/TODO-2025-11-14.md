...
- Estimates: cost-code-only invoicing (no materials), enforce cost_code_id required on estimate, polish form UX across Laravel editions
...
- Estimate create form: on some environments, Blade printed @selected/expressions in Cost Code dropdown. We replaced with conservative Blade, but issue persists for some setups; needs further investigation into view compilation/version differences.
