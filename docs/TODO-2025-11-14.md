...
- Upsert debug + isolation: added logging with intuit_tid; next: send minimal sparse payload with only changed, allowed fields (PrimaryPhone/PrimaryEmailAddr/BillAddr) to resolve 2010

## Next Up
- Token refresh-on-demand (retry once on 401) and proactive refresh for background jobs
- Minimal sparse updates for Customer (exclude DisplayName/CompanyName/GivenName/FamilyName/Mobile on update; include only changed fields)
- Capture intuit_tid header in logs for QBO troubleshooting (expand to all QBO calls)
...
- Outbound 2010 ValidationFault on Customer update in Production: QBO rejects some properties in sparse updates; implement minimal payload strategy and retest (Mobile likely culprit)
