...
- Address Autocomplete polish (High)
  - Normalize font/height across browsers; ensure consistent 16–18px text in picker.
  - Confirm robust parsing for non-US formats; improve fallback parsing where addressComponents are partial.
  - Add optional map preview under the picker (deferred).
- Place Picker wiring (High)
  - Ensure no conflicting loaders exist (classic Maps JS). Keep only Extended Component Library.
