...
- Invoices: Phase 1 – Create in QBO using single “Services” item (non-taxed), link to Customer, store qbo_invoice_id/sync token; inbound Refresh from QBO to update status/balance. Auto-create Services Item if missing with robust Account fallback.

## Next Up
- Invoices: Webhook + CDC ingestion for Invoice status/balance; add “View in QBO” link; optional push updates pre‑payment.
- QBO settings: allow selecting an existing QBO Item for invoices (skip auto-create), or pick an Income account for auto-create.
